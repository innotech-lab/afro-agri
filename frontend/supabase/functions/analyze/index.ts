// Public diagnose API alias — POST /functions/v1/analyze
// Same contract as /diagnose: header x-api-key, body { image_base64, crop? }
...
Deno.serve(async (req) => {
  ...
  const fp = await sha256(String(body.image_base64).slice(0, 256));
  const speciesIdx = parseInt(fp.slice(0, 2), 16) % SPECIES.length;
  const dKeys = Object.keys(TREATMENTS);
  const dKey = dKeys[parseInt(fp.slice(2, 4), 16) % dKeys.length];
